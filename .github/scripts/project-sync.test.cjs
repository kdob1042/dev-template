...
const fullName = 'kdob1042/dev-template';
const issue = { number: 10, node_id: 'I10', state: 'open', labels: [] };
const pull = {
  number: 13,
  state: 'open',
  body: 'Refs #10',
  head: { ref: 'issue/10-work', sha: 'new', repo: { full_name: fullName } },
  base: { ref: 'dev' }
};

test('settings default to dev and the repository owner project', () => {
  const settings = settingsFor({ repo: { owner: 'kdob1042', repo: 'dev-template' }, payload: {} });
  assert.equal(settings.workBranch, 'dev');
  assert.equal(settings.projectOwner, 'kdob1042');
  assert.equal(settings.projectNumber, 1);
  assert.equal(settings.ciName, '');
});

test('explicit local references only; completion wins over Refs', () => {
  assert.deepEqual([...references('Refs #10\nCloses #10\n- Fixes kdob1042/dev-template#11', fullName)], [[10, true], [11, true]]);
  const fence = String.fromCharCode(96).repeat(3);
  const inline = String.fromCharCode(96) + 'Closes #10' + String.fromCharCode(96);
  for (const body of [
  ...
  assert.equal(related({ ...pull, head: { ...pull.head, repo: { full_name: 'fork/dev-template' } } }, 10, fullName), false);
});

test('partial work requires an explicit follow-up Issue marker', () => {
  assert.deepEqual([...followUpReferences('Parent: #10', fullName)], [10]);
  assert.deepEqual([...followUpReferences('Continues: kdob1042/dev-template#10', fullName)], [10]);
...
test('only explicit complete merge or migrated follow-up closes; historical and multiple PR cases stay open', () => {
  ...
  for (const candidate of [
    ...
    { ...merged, merged_at: '2026-09-16T00:00:00Z' },
    { ...merged, base: { ref: 'feature' } }
  ]) {
...
test('CI recovery uses latest run/attempt and current head, ignores unrelated workflows', () => {
  ...
  assert.equal(failingRuns([bad], 'new', ''), false);
});
...
function fixture({ status = 'Todo', prs = [], state = 'open', events = [], access = true, archived = false, labels = [], exists = true } = {}) {
  ...
  const item = {
    id: 'item',
    isArchived: archived,
    content: { __typename: 'Issue', id: 'I10', number: 10, repository: { nameWithOwner: fullName } },
    fieldValueByName: { name: status }
  };
  const opts = ['Todo', 'In Progress', 'Needs attention', 'Done'].map(name => ({ name, id: name }));
  const github = {
    rest: { issues: {}, pulls: {}, actions: {} },
    paginate: async method => method(),
    graphql: async (query, variables) => {
      if (query.includes('viewerCanUpdate')) {
        return { user: { projectV2: { id: 'project', viewerCanUpdate: access, fields: { nodes: [{ id: 'field', name: 'Status', options: opts }] } } } };
      }
      if (query.includes('items(first')) {
        return { node: { items: { nodes: exists ? [item] : [], pageInfo: { hasNextPage: false } } } };
      }
  ...
  const context = { repo: { owner: 'kdob1042', repo: 'dev-template' }, payload: {} };
